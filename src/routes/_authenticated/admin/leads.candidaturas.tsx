...
import {
  deleteJobApplicationsFn,
  listJobApplicationsFn,
  updateJobApplicationFn,
} from "@/fns/careers/admin";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import { CAREERS_STATUS_LABEL } from "@/lib/careers";

export const Route = createFileRoute("/_authenticated/admin/leads/candidaturas")({
  component: Page,
});

type Application = Awaited<ReturnType<typeof listJobApplicationsFn>>["applications"][number];
...
function Page() {
  ...
  const load = async () => {
    try {
      const result = await listJobApplicationsFn({
        data: {
      ...
      toast.error(error instanceof Error ? error.message : "Erro ao carregar candidaturas.");
    }
  };
  ...
  const deleteSelected = async () => {
    ...
    setDeleting(true);
    try {
      await deleteJobApplicationsFn({ data: { ids: [...selectedIds] } });
      toast.success(
        selectedIds.size === 1 ? "Candidatura excluída." : `${selectedIds.size} candidaturas excluídas.`,
      );
      setSelectedIds(new Set());
      setConfirmDelete(false);
      load();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Erro ao excluir candidaturas.");
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="mx-auto max-w-6xl space-y-6 p-6 lg:p-10">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="font-serif text-3xl">👥 Trabalhe Conosco</h1>
          <p className="text-sm text-muted-foreground">
            Candidaturas recebidas pela página pública /trabalhe-conosco
          </p>
        </div>
            ...
            <CardContent className="flex flex-wrap items-center justify-between gap-3 px-5 py-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {app.email} · {app.whatsapp} · {app.city}
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {app.role_interest} · {format(new Date(app.created_at), "dd/MM/yyyy HH:mm")}
                  </p>
                </div>
              </div>
              <Badge variant="outline">{CAREERS_STATUS_LABEL[app.status] || app.status}</Badge>
            </CardContent>
          </Card>
        ))}
        {applications.length === 0 && (
            ...
            <>
              <DialogHeader className="border-b bg-card px-6 pb-4 pt-6">
                <div className="flex flex-wrap items-start justify-between gap-3 pr-6">
                  <div>
                    <DialogTitle className="font-serif text-2xl">{selected.full_name}</DialogTitle>
                    <DialogDescription className="mt-1">
                      Recebido em {format(new Date(selected.created_at), "dd/MM/yyyy HH:mm")}
                    </DialogDescription>
                  </div>
                  <Badge variant="outline">
                    {CAREERS_STATUS_LABEL[selected.status] || selected.status}
                  </Badge>
                </div>
              </DialogHeader>
                  ...
                  {selected.birth_date ? (
                    <div className="grid grid-cols-[minmax(7rem,38%)_1fr] gap-3 px-4 py-3">
                      ...
                      <dd className="font-medium">
                        {format(new Date(selected.birth_date), "dd/MM/yyyy")}
                      </dd>
                    </div>
                  ) : null}
                  <div className="grid grid-cols-[minmax(7rem,38%)_1fr] gap-3 px-4 py-3">
                    <dt className="text-muted-foreground">Função de interesse</dt>
                    <dd className="font-medium">{selected.role_interest}</dd>
                  </div>
                  <div className="grid grid-cols-[minmax(7rem,38%)_1fr] gap-3 px-4 py-3">
                    <dt className="text-muted-foreground">Experiência</dt>
                    <dd className="font-medium">
                      {selected.has_experience ? "Sim" : "Não"}
                      {selected.experience_details ? ` — ${selected.experience_details}` : ""}
                    </dd>
                  </div>
                  <div className="grid grid-cols-[minmax(7rem,38%)_1fr] gap-3 px-4 py-3">
                    <dt className="text-muted-foreground">Disponibilidade</dt>
                    <dd className="font-medium">{selected.availability.join(", ") || "—"}</dd>
                  </div>
                  {selected.additional_info ? (
                    <div className="grid grid-cols-[minmax(7rem,38%)_1fr] gap-3 px-4 py-3">
                      <dt className="text-muted-foreground">Informações adicionais</dt>
                      <dd className="font-medium">{selected.additional_info}</dd>
                    </div>
                  ) : null}
                </dl>
      ...
      <AlertDialog open={confirmDelete} onOpenChange={(v) => !v && setConfirmDelete(false)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Excluir {selectedIds.size === 1 ? "candidatura" : `${selectedIds.size} candidaturas`}?
            </AlertDialogTitle>
            <AlertDialogDescription>
